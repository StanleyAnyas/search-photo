            ...
            <Grid container spacing={{ xs: 4, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }} sx={{paddingLeft: 5, paddingBottom: 7}}>
                {images.map((image) => (
                    <Grid item xs={4} sm={4} md={4} key={image.id}>
                        <Card sx={{ maxWidth: 550, height: 550, boxShadow: "0 8px 16px rgba(0, 0, 0, 0.3)" }}>
                            <CardMedia
                                component="img"
                                height="300"
                                image={image.url}
                            ...
                            <CardContent sx={{flexGrow: 1, textAlign: "left", position: "relative"}}>
                                {<span>
                                 <Typography variant="body2" sx={{ fontSize: 25, fontFamily: "@fontsource/roboto/900.css", textAlign: "left", color: "black", fontWeight: "bold"}} color="text.secondary">
                                    {image.nameOfPicture.length > 30 ? image.nameOfPicture.substring(0, 30) + "..." : image.nameOfPicture}
                                </Typography> </span>}
                                <br />
                                {<span> 
                                <Typography variant="body2" sx={{fontSize: 20, textAlign: "left", color:  "bold"}} color="text.secondary">
                                    {image.description.length > 40 ? image.description.substring(0, 40) + "..." : image.description}
                                </Typography></span>}   
                                <br /> 
                                {<span>
                                <Typography variant="body2" sx={{ fontSize: 15, textAlign: "left", color:  "bold"}} color="text.secondary">
